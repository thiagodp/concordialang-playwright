...
import path, { relative, resolve, dirname, basename, join } from 'path';
import fs, { writeFile as writeFile$1 } from 'fs';
  ...
  xfs.mkdir(p, mode, er => {
    ...
    switch (er.code) {
      case 'ENOENT':
        ...
        mkdirs$2(path.dirname(p), opts, (er, made) => {
...
function mkdirsSync$1 (p, opts, made) {
  ...
  try {
    ...
    if (err0.code === 'ENOENT') {
      ...
      made = mkdirsSync$1(path.dirname(p), opts, made);
...
function makeSourceCodeFilePath(outputDirectory, specificationFilePath, specificationBaseDirectory, fileExtension) {
  const relativeSpecFilePath = relative(specificationBaseDirectory, specificationFilePath);
  const correspondingOutputDir = resolve(outputDirectory, dirname(relativeSpecFilePath));
  const fileName = basename(relativeSpecFilePath, '.testcase') + fileExtension;
  const filePath = join(correspondingOutputDir, fileName);
  return filePath;
}
function ensureSourceCodeFileDirectory(filePath) {
  lib.mkdirsSync(dirname(filePath));
}
async function saveFile(path, content) {
  const write = promisify(writeFile$1);
...
function couldNotConvertCommand(filePath, location) {
  warn('Plug-in could not convert command from', colors.yellowBright(filePath), location ? '(' + location.line + ',' + location.column + ')' : '');
}
